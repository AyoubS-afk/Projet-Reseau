...
class GameView(arcade.View):
    ...
    def __init__(self, _game, name="save_1"):
        ...
        # =======================================
        self.tab = arcade.load_texture(
            constantes.SPRITE_PATH + "PanelsOther/paneling_00017.png")
        self.bar = arcade.load_texture(
            constantes.SPRITE_PATH + "Panel/panel0.png")
        self.money_box = arcade.load_texture(
            constantes.SPRITE_PATH + "PanelsOther/paneling_00015.png")
        ...
        self.load_button = UI_buttons.Text_Button_background(
            x=40, y=constantes.DEFAULT_SCREEN_HEIGHT - 3*self.bar.height/4, width=20, height=self.bar.height, texture=None, my_text="Load Game", color="black")
        self.load_button.on_click = self.button_load_on_click
        self.layer_button = UI_buttons.Text_Button_background(x=constantes.DEFAULT_SCREEN_WIDTH-162 + 3, y=(3*constantes.DEFAULT_SCREEN_HEIGHT/4 -
                                                              self.bar.image.size[1]/2) + constantes.DEFAULT_SCREEN_HEIGHT/2 * 0.4444444444, width=120, height=25, texture=UI_buttons.texture_panel11, my_text="Overlays", color="black")
        ...
        self.normal_layer_button = UI_buttons.Text_Button_background(
            x=constantes.DEFAULT_SCREEN_WIDTH-282, y=constantes.DEFAULT_SCREEN_HEIGHT - self.bar.height - 1, width=120, height=25, texture=UI_buttons.texture_panel11, my_text="Normal", color="black")
        self.layer_manager.add(self.normal_layer_button)
        self.normal_layer_button.on_click = self.button_normal_layer_on_click
        self.fire_layer_button = UI_buttons.Text_Button_background(x=constantes.DEFAULT_SCREEN_WIDTH-282, y=constantes.DEFAULT_SCREEN_HEIGHT -
                                                                   self.bar.height - 26, width=120, height=25, texture=UI_buttons.texture_panel11, my_text="Fire", color="black")
        self.layer_manager.add(self.fire_layer_button)
        self.fire_layer_button.on_click = self.button_fire_layer_on_click
        self.collapse_layer_button = UI_buttons.Text_Button_background(
            x=constantes.DEFAULT_SCREEN_WIDTH-282, y=constantes.DEFAULT_SCREEN_HEIGHT - self.bar.height - 51, width=120, height=25, texture=UI_buttons.texture_panel11, my_text="Collapse", color="black")
        ...
        self.fps_up = arcade.gui.UITextureButton(x=constantes.DEFAULT_SCREEN_WIDTH - 162 + 10, y=constantes.DEFAULT_SCREEN_HEIGHT -
                                                 self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2, texture=arcade.load_texture(constantes.SPRITE_PATH + "Panel/Panel42/paneling_00249.png"), width=25, height=15)
        self.fps_down = arcade.gui.UITextureButton(x=constantes.DEFAULT_SCREEN_WIDTH - 162 + 35, y=constantes.DEFAULT_SCREEN_HEIGHT -
                                                   self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2, texture=arcade.load_texture(constantes.SPRITE_PATH + "Panel/Panel43/paneling_00253.png"), width=25, height=15)
    ...
    def setup(self):
        ...
        self.money_text = text.Sprite_sentence(
            "Dn: " + str(self.game.money), "white", (205, constantes.DEFAULT_SCREEN_HEIGHT-self.bar.image.size[1]/4))
        self.fps_text = text.Sprite_sentence(str(self.speed_ratio) + "%", "black", (constantes.DEFAULT_SCREEN_WIDTH -
                                             162 + 85, constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2 + 10))
        self.population_text = text.Sprite_sentence("Pop :" + str(constantes.WALKER_UNIT*len(
            self.game.walkersAll)), "white", (505, constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1]/4))
        self.fps_text2 = text.Sprite_sentence("Pop :" + str(self.game.framerate), "white", (605 - (
            len(self.population_text.sentence)), constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1]/4))
        self.visualmap.setup(self.game)
        self.center_map()
        self.minimap_camera.move_to(self.visualmap.get_map_center(
        ) - Vec2(self.window.width/2, self.window.height/2), 1)
        ...
        self.minimap_sprite = arcade.Sprite(center_x=(constantes.DEFAULT_SCREEN_WIDTH-MINIMAP_WIDTH/2 - 9),
                                            center_y=(
                                                3*constantes.DEFAULT_SCREEN_HEIGHT/4 - self.bar.image.size[1]/2 + 0.3044444444*constantes.DEFAULT_SCREEN_HEIGHT/2),
    ...
    def update_minimap(self):
        ...
        proj = self.minimap_camera.position[0]-(15*map_width/100), self.minimap_camera.position[0]+(85*map_width/100), \
            self.minimap_camera.position[1]-(
                15*map_height/100), self.minimap_camera.position[1]+(85*map_height/100)
    ...
    def on_draw(self):
            ...
            if self.builder_mode:
                if not self.mouse_left_maintained:
                    if self.builder_content != "road":
                        hollow = hudb.hollow_build(
                            self.mouse_pos[0], self.mouse_pos[1], self.visualmap, gdata.building_dico[self.builder_content.lower()])
                    else:
                        hollow = hudb.hollow_build(
                            self.mouse_pos[0], self.mouse_pos[1], self.visualmap)
                    hollow.draw()
                else:
        ...
        arcade.draw_texture_rectangle(center_x=constantes.DEFAULT_SCREEN_WIDTH / 2,
                                      center_y=constantes.DEFAULT_SCREEN_HEIGHT -
                                      self.bar.image.size[1]/4,
        ...
        arcade.draw_texture_rectangle(center_x=constantes.DEFAULT_SCREEN_WIDTH - 81,
                                      center_y=3*constantes.DEFAULT_SCREEN_HEIGHT /
                                      4 - self.bar.image.size[1]/2,
                                      width=162, height=constantes.DEFAULT_SCREEN_HEIGHT / 2,
                                      texture=self.tab
                                      )
        arcade.draw_texture_rectangle(center_x=constantes.DEFAULT_SCREEN_WIDTH - 81, center_y=constantes.DEFAULT_SCREEN_HEIGHT -
                                      self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2-23 - 50, width=162, height=200, texture=arcade.load_texture(constantes.SPRITE_PATH + "Panel/panel46.png"))
        arcade.draw_texture_rectangle(center_x=constantes.DEFAULT_SCREEN_WIDTH - 81, center_y=constantes.DEFAULT_SCREEN_HEIGHT -
                                      self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2-23 - 50 - 200, width=162, height=200, texture=arcade.load_texture(constantes.SPRITE_PATH + "Map_panels/map_panels_00002.png"))
    ...
    def on_update(self, delta_time: float):
        if self.is_paused:
            ...
            for sprite in self.visualmap.fire_layer:
                sprite.set_texture(self.visualmap.fire_count %
                                   len(sprite.textures))

            self.move_map_camera_with_keys()
            self.visualmap.update_walker_list(self.game.walkersOut)
            self.money_text = text.Sprite_sentence("Dn: " + str(self.game.money), "white", (320-(len(
                self.money_text.sentence)+5) * constantes.FONT_WIDTH/4, constantes.DEFAULT_SCREEN_HEIGHT-self.bar.image.size[1]/4))
            self.fps_text = text.Sprite_sentence(str(self.speed_ratio) + "%", "black", (constantes.DEFAULT_SCREEN_WIDTH -
                                                 162 + 85, constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1] - constantes.DEFAULT_SCREEN_HEIGHT/2 + 10))
            self.population_text = text.Sprite_sentence("Pop :" + str(constantes.WALKER_UNIT*len(self.game.walkersAll)), "white", (505 - (
                len(self.population_text.sentence)), constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1]/4))
            self.fps_text2 = text.Sprite_sentence("Fps:" + str(1/delta_time), "white", (605 - (len(
                self.population_text.sentence)), constantes.DEFAULT_SCREEN_HEIGHT - self.bar.image.size[1]/4))
    # =======================================
            ...
            if button == arcade.MOUSE_BUTTON_LEFT:
                ...
                if not self.builder_mode and not self.remove_mode and not self.builder_content == "road":
                    ...
                    if not nearest_sprite.textures:
                        ...
                        (i, j) = self.visualmap.get_sprite_at_screen_coordinates(
                            nearest_sprite.position)
                        bat = self.game.map.buildings_layer.get_cell(i, j)
                        print(bat.dic["version"])
                        k, p = 0, 0
                        self.actual_pop_up = pop.info_building_pop_up(
                            bat.dic["version"], bat.current_number_of_employees, bat.max_number_of_employees)
            ...
            if button == arcade.MOUSE_BUTTON_LEFT:
                if self.remove_mode:
                    if self.mouse_left_maintained:
                        self.remove_elements_serie(
                            self.init_mouse_pos, (Vec2(x, y) + self.map_camera.position))
                        pos1, pos2 = self.visualmap.get_sprite_at_screen_coordinates(
                            self.init_mouse_pos)
                        pos3, pos4 = self.visualmap.get_sprite_at_screen_coordinates(
                            (Vec2(x, y) + self.map_camera.position))
                        self.game.send_buff = game.interNet.net.encrypter_msg(
                            "remove_element_serie", str(pos1), str(pos2), str(pos3), str(pos4))
                        self.dragged_sprite.clear()
                    else:
                        self.remove_sprite(self.mouse_pos)
                        line, column = self.visualmap.get_sprite_at_screen_coordinates(
                            self.mouse_pos)
                        self.game.send_buff = game.interNet.net.encrypter_msg(
                            "remove_sprite", str(line), str(column))
                else:
                ...
                if self.builder_mode:
                    if self.mouse_left_maintained:
                        if self.builder_content == "dwell":
                            ...
                            pos1, pos2, pos3, pos4 = self.surface_drag[0][0], self.surface_drag[
                                0][1], self.surface_drag[-1][0], self.surface_drag[-1][1]
                            version = gdata.building_dico[self.builder_content].name
                            self.game.send_buff = game.interNet.net.encrypter_msg(
                                "add_multiple_one_sized_building", str(pos1), str(pos2), str(pos3), str(pos4), version)
                            self.dragged_sprite.clear()
                        elif self.builder_content == "road":
                            tmp_end_pos = Vec2(x, y) + self.map_camera.position
                            self.add_roads_serie(
                                self.init_mouse_pos, tmp_end_pos)
                            pos1, pos2 = self.visualmap.get_sprite_at_screen_coordinates(
                                self.init_mouse_pos)
                            pos3, pos4 = self.visualmap.get_sprite_at_screen_coordinates(
                                tmp_end_pos)
                            self.game.send_buff = game.interNet.net.encrypter_msg(
                                "add_road_serie", str(pos1), str(
                                    pos2), str(pos3), str(pos4), str(0))
                    else:
                        if self.builder_content == "road":
                            ...
                            version = gdata.building_dico[self.builder_content].name
                            self.game.send_buff = game.interNet.net.encrypter_msg(
                                "add_building", str(line), str(column), version)
        ...
        if self.mouse_left_pressed:
            if self.builder_mode:
                if self.builder_content == "road":
                    if self.mouse_left_maintained:
                        self.add_roads_serie(
                            self.init_mouse_pos, tmp_end_pos, True)
                        pos1, pos2 = self.visualmap.get_sprite_at_screen_coordinates(
                            self.init_mouse_pos)
                        pos3, pos4 = self.visualmap.get_sprite_at_screen_coordinates(
                            tmp_end_pos)
                        self.game.send_buff = game.interNet.net.encrypter_msg(
                            "add_road_serie", str(pos1), str(
                                pos2), str(pos3), str(pos4), str(1))
                else:
                    self.get_surface_dragged(self.init_mouse_pos, tmp_end_pos)
                    self.dragged_sprite.clear()
                    self.visualmap.fill_temporary_build(
                        self.surface_drag, self.dragged_sprite, self.builder_content, "build")
            if self.remove_mode:
                self.get_surface_dragged(self.init_mouse_pos, tmp_end_pos)
                self.dragged_sprite.clear()
                self.visualmap.fill_temporary_build(
                    self.surface_drag, self.dragged_sprite, self.builder_content, "remove")
    ...
    def center_scroll_to(self, position):
        ...
        :return:
        """
        self.map_camera.move_to(
            position - Vec2(self.window.width / 2, self.window.height / 2), 1)
    ...
    def add_road(self, pos) -> bool:
        ...
        if self.game.add_road(line, column):
            ...
            self.visualmap.update_layers(
                self.visualmap.roads_layer, self.game.map.roads_layer.array)
            return True
        return False

    def add_road_2(self, pos) -> bool:
        """
        Fonction d'ajout de route
        va probablement être transformée en fonction plus générale d'ajout d'élément
        """
    ...
    def add_roads_serie(self, start_pos, end_pos, dynamically=False) -> bool:
        ...
        """
        line1, column1 = self.visualmap.get_sprite_at_screen_coordinates(
            start_pos)
        line2, column2 = self.visualmap.get_sprite_at_screen_coordinates(
            end_pos)
    ...
    def add_roads_serie_2(self, start_pos, end_pos, dynamically=False) -> bool:
        """
        Fonction qui permet d'ajouter une série de routes
        Prend en paramètre 2 positions de souris sous forme de tuple
        """
    ...
    def add_one_sized_building(self, pos):
        ...
        if self.game.add_building(line, column, building):
            self.visualmap.update_layers(
                self.visualmap.buildings_layer, self.game.map.buildings_layer.array)
            return True
        return False
    ...
    def add_multiple_one_sized_building_2(self, pos1, pos2, pos3, pos4, building):
        tile_list = list()
        if pos3 < pos1:
            pos1, pos3 = pos3, pos1
        if pos4 < pos2:
            pos2, pos4 = pos4, pos2
        for i in range(pos1, pos3+1):
            for j in range(pos2, pos4+1):
                tile_list.append((i, j))
        for (line, column) in tile_list:
            if self.game.add_building(line, column, building):
                pass
            else:
                print("Building failed")
    ...
    def remove_sprite(self, pos) -> bool:
        ...
        if what_is_removed == constantes.LAYER4:
            ...
            self.visualmap.update_layers(
                self.visualmap.trees_layer, self.game.map.trees_layer.array)
            return True
        return False

    def remove_sprite_2(self, pos) -> bool:
        line, column = pos[0], pos[1]
        what_is_removed = self.game.remove_element((line, column))
        if what_is_removed == constantes.LAYER4:
            self.visualmap.update_layers(
    ...
    def remove_elements_serie(self, start_pos, end_pos) -> bool:
        ...
        line2, column2 = self.visualmap.get_sprite_at_screen_coordinates(
            end_pos)
        modified_layers = self.game.remove_elements_serie(
    ...
    def remove_elements_serie_2(self, start_pos, end_pos) -> bool:
        """
        Pour clean une surface de la carte
        Cette fonction doit être associée au bouton pelle
        """
    ...
    def draw_message_for_farm_building(self):
        ...
        start_x = (constantes.DEFAULT_SCREEN_WIDTH -
                   error_message.content_width) // 2
    ...
    def attribute_on_click(self):
        l = ["", "", "", "", "", "", "", "", "water", "health", "religion",
             "roll", "entertainment", "education", "hammer", "sword", "carry"]
        for i in range(0, len(l)):
            if i > 7:
                self.buttons[i].on_click = UI_buttons.define_on_click_button_manager(
                    self, l[i])
    ...
    def update_treatment(self, update: updates.LogicUpdate):
        ...
        for j in update.catchedfire:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=j, update_type="building_fire")
        for k in update.collapsed:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=k, update_type="building_destroy")
        # Devolve before evolved -- order is important
        for m in update.has_devolved:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=m[0], update_type="stat_dec", special_value=m[1])
        for l in update.has_evolved:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=l[0], update_type="stat_inc", special_value=l[1])

        for n in update.removed:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=n, update_type="delete")
        for briskfire in update.fire_level_change:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=briskfire[0], update_type="risk_update", special_value=("fire", briskfire[1]))
        for briskcollapse in update.collapse_level_change:
            self.visualmap.update_one_sprite(
                layer=self.visualmap.buildings_layer, position=briskcollapse[0], update_type="risk_update", special_value=("collapse", briskcollapse[1]))
